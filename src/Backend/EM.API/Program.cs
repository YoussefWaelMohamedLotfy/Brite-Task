...
builder
    ...
    .AddPolicy(
        "Viewer-Policy",
        x => x.RequireAuthenticatedUser().AddRequirements(new RoleRequirement("Viewer"))
    )
    .AddPolicy(
        "Admin-Policy",
        x => x.RequireAuthenticatedUser().AddRequirements(new RoleRequirement("Admin"))
    )
    .AddPolicy(
        "HR-Viewer-Policy",
        x => x.RequireAuthenticatedUser().AddRequirements(new RoleRequirement("HR", "Viewer"))
    )
    .AddPolicy(
        "HR-Admin-Policy",
        x => x.RequireAuthenticatedUser().AddRequirements(new RoleRequirement("HR", "Admin"))
    )
    .AddPolicy(
        "Viewer-Admin-Policy",
        x => x.RequireAuthenticatedUser().AddRequirements(new RoleRequirement("Viewer", "Admin"))
    )
    .AddPolicy(
        "HR-Viewer-Admin-Policy",
        x =>
            x.RequireAuthenticatedUser()
                .AddRequirements(new RoleRequirement("HR", "Viewer", "Admin"))
    );
...
builder.Services.AddOpenApi(x =>
{
        ...
        Flows = new()
        {
            AuthorizationCode = new OpenApiOAuthFlow
            {
                AuthorizationUrl = new Uri(
                    "http://localhost:8081/realms/tenant-1/protocol/openid-connect/auth"
                ),
                TokenUrl = new Uri(
                    "http://localhost:8081/realms/tenant-1/protocol/openid-connect/token"
                ),
    ...
    x.AddDocumentTransformer(
        ...
        {
            document.Info.Contact = new() { Name = "Brite Support", Email = "info@gobrite.ai" };
...
app.MapScalarApiReference(x =>
    x.WithTheme(ScalarTheme.Default)
